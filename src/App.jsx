...
export default function App() {
  ...
  // ── Cart (custom hook) ───────────────────────────────────────
  const {
    cart, discount, setDiscount, taxEnabled, setTaxEnabled,
  ...
  const [printMode,          setPrintMode]         = useState(null); // 'receipt'|'reprint'|'report'
  ...
  useEffect(() => {
    ...
    fetch(url).then(r => r.ok ? r.json() : []).then(files => {
      if (!Array.isArray(files)) return;
      const imgs = files.filter(f => /\.(jpe?g|png|webp|gif)$/i.test(f.name)).map(f => f.download_url);
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      @media print {
        ...
        /* Receipt mode: show ONLY receipt div */
        ...
        @page:print-receipt { size: 72mm auto; margin: 2mm; }
      }
    `;
  ...
  const handlePrintReceipt = useCallback(async (trx = lastTransaction, isReprint = false) => {
    ...
    if (printer.settings.method === 'browser') {
      // Browser method: inject receipt into DOM, trigger print
      ...
      // Non-browser (Bluetooth/RawBT/Network): direct bytes
      try {
  ...
  const handleLogin = (e) => {
    ...
    if (loginTargetUser.role === 'kasir') {
      const hasShift = shifts.find(s => s.userId === loginTargetUser.id && s.status === 'open');
    ...
    try {
      ...
      if (!editingTransactionId) {
        ...
        cart.forEach(item => {
          ...
          batch.update(doc(getColRef('products'), String(item.id)), { stock: newStock });
          ...
          batch.set(doc(getColRef('stockLogs'), logId), {
            ...
            qtyChange: -(item.qty || 0), finalStock: newStock,
        ...
        allIds.forEach(id => {
          ...
          const finalStock    = master.stock - delta;
          ...
          if (delta < 0 && finalStock > 0) {
            updatedCost = ((master.stock * master.cost) + (qtyReturned * oldItem.cost)) / finalStock;
          }

          batch.update(doc(getColRef('products'), String(id)), { stock: finalStock, cost: updatedCost });
          const logId = makeLogId();
          batch.set(doc(getColRef('stockLogs'), logId), {
            id: logId, productId: id, productName: master.name,
            type: logType, reason: logReason,
            qtyChange: -delta, finalStock,
  ...
  const handleAddProduct = async (e) => {
    ...
    const batch = writeBatch(db);
    const prodData = { id, ...newItem, price: Number(newItem.price), cost: Number(newItem.cost), stock: Number(newItem.stock) };
  ...
  const handleProcessUpdateStock = async (e) => {
    ...
    if (addedQty > 0 && stockUpdateReason === 'Restock Barang Masuk') {
      ...
      updatedCost = ((oldQty * oldCost) + (addedQty * nc)) / finalQty;
    }

    const batch = writeBatch(db);
    batch.update(doc(getColRef('products'), String(stockUpdateItem.id)), { stock: finalQty, cost: updatedCost });
  ...
  const processPayDebt = async (e) => {
    ...
    await batch.commit();
    setPayDebtModalOpen(false); setPayDebtAmount('');
  ...
  const handleAddKasir = async (e) => {
    ...
    await setDoc(doc(getColRef('users'), id), { id, role: 'kasir', name: newKasirName.trim(), password: '008' });
    setNewKasirName('');
  ...
  const openPasswordModal = (user) => {
    showPrompt('Ubah Kata Sandi', `Sandi baru untuk ${user.name}:`, '', 'Sandi baru...', async (pw) => {
      if (pw) { await updateDoc(doc(getColRef('users'), user.id), { password: pw }); showAlert('✅ Sandi diperbarui.'); }
    });
  };
  ...
  const handleChangeTheme = async (key) => {
    if (currentUser?.role !== 'admin') { showAlert('Hanya Administrator yang dapat mengubah tema.'); return; }
  ...
  const resumeHoldBill = async (bill) => {
    clearCart();
    // Load bill items into cart manually (bypass hook's addToCart stock check)
    const { setCart } = useCart.__proto__ || {};  // noop — done via direct set below
    // We need to use the cart setter from the hook
    // The hook exposes setCart indirectly via loadEditTransaction for trx
    // For hold bills, we'll use a workaround approach
    // This is handled in the POSScreen/DashboardPanels by calling the exposed handlers
    await import('./firebase').then(({ db }) => {
      const { deleteDoc, doc } = require('firebase/firestore');
      deleteDoc(doc(getColRef('holdBills'), bill.id));
    });
    setActiveTab('pos');
  };
  ...
  if (!isDbReady) {
    return (
      <div className="flex flex-col items-center justify-center h-[100dvh] w-full bg-[#1a1412]">
        ...
        <p className="mt-4 text-amber-500/80 font-mono tracking-widest text-sm">INISIALISASI SISTEM ENTERPRISE...</p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-center h-[100dvh] w-full text-amber-50 font-sans relative overflow-hidden">
        ...
        <div className="fixed inset-0 z-[-1] bg-[#1a1412]">
          {backgroundImages.map((img, idx) => (
            <img key={idx} src={img} alt="" className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${idx === currentBg ? 'opacity-40' : 'opacity-0'}`} />
          ))}
        ...
        <div className="bg-[#221815]/80 backdrop-blur-xl border border-red-900/50 rounded-3xl p-8 max-w-sm w-full z-10 shadow-2xl text-center">
          ...
          {loginStep === 'role' && (
            <div className="space-y-3">
              <button onClick={() => { setLoginTargetUser(users.find(u => u.id === 'admin')); setLoginStep('password'); }}
              ...
              <button onClick={() => setLoginStep('kasir-list')}
              ...
              <button onClick={() => { setLoginTargetUser(users.find(u => u.id === 'owner')); setLoginStep('password'); }}
                className="w-full flex items-center gap-3 px-4 py-3 bg-[#2a1f1b]/80 hover:bg-[#30231f] border border-amber-900/50 text-amber-400 rounded-xl font-bold transition-all">
            ...
            <div className="space-y-3">
              ...
              <div className="max-h-48 overflow-y-auto space-y-2 pr-1">
                {users.filter(u => u.role === 'kasir').map(k => (
                  <button key={k.id} onClick={() => { setLoginTargetUser(k); setLoginStep('password'); }}
              ...
              <button onClick={() => setLoginStep('role')} className="w-full flex items-center justify-center gap-2 text-sm text-amber-500 hover:text-amber-300 py-2">
          ...
          {loginStep === 'password' && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              {loginError && <p className="text-xs text-red-400 font-bold animate-pulse">{loginError}</p>}
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => { setLoginStep('role'); setLoginError(''); }}
  ...
  const dashboardProps = {
    ...
    resumeHoldBill: async (bill) => {
      // Direct cart load for hold bills
      clearCart();
      // Import and use the cart's internal setter via window event
      window.dispatchEvent(new CustomEvent('gg-load-cart', { detail: bill.items }));
      const { deleteDoc: del, doc: docFn } = await import('firebase/firestore');
      await del(docFn(getColRef('holdBills'), bill.id));
      setActiveTab('pos');
    },
  ...
  return (
    <div className={`flex h-[100dvh] w-full font-sans overflow-hidden ${theme.textMain}`}>
      ...
      <div className="fixed inset-0 z-[-1]">
        {backgroundImages.map((img, idx) => (
          <img key={idx} src={img} alt="" className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${idx === currentBg ? 'opacity-100' : 'opacity-0'}`} />
        ))}
        ...
        <div className={`p-4 border-t ${theme.border} space-y-2 bg-black/10`}>
          ...
          <div className="flex items-center gap-2 px-2 mb-1">
            ...
            {currentUser?.role === 'admin' ? (
              <select value={currentThemeKey} onChange={e => handleChangeTheme(e.target.value)}
                className={`w-full bg-transparent ${theme.textMain} text-sm focus:outline-none cursor-pointer font-semibold`}>
                {Object.keys(themes).map(k => (
                  <option key={k} value={k} className="bg-[#1a1412] text-amber-50">{themes[k].name}</option>
                ))}
              </select>
            ) : (
              <span className={`text-sm ${theme.textMain} font-semibold`}>Tema: {themes[currentThemeKey]?.name}</span>
            )}
          </div>
          ...
          {currentUser?.role === 'kasir' && (
            <button onClick={() => { setShiftAction('close'); setShiftCashInput(''); setShiftModalOpen(true); }}
      ...
      <main className="flex-1 flex flex-col h-full z-10 relative min-w-0">
        ...
        <header className={`lg:hidden p-4 ${theme.bgPanel} backdrop-blur-md border-b ${theme.border} flex justify-between items-center shrink-0`}>
          <button onClick={() => setIsSidebarOpen(true)} className={`p-2 hover:bg-black/20 rounded-lg ${theme.textMain}`}>
            <MenuIcon size={24} />
          </button>
          ...
          {activeTab === 'pos' && ['admin', 'kasir'].includes(currentUser?.role) ? (
            <button onClick={() => setIsCartOpen(true)} className={`relative p-2 hover:bg-black/20 rounded-lg ${theme.textMain}`}>
      ...
      <PrintSettingsModal isOpen={printSettingsOpen} onClose={() => setPrintSettingsOpen(false)} theme={theme} />
