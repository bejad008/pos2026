...
export default function App() {
  ...
  // ── Cart (custom hook) ───────────────────────────────────────
  const {
    cart, setCart,             // ✅ FIX: destructure setCart untuk resumeHoldBill
  ...
  // ✅ FIX: handleFinishTransaction harus didefinisikan SEBELUM
  // handlePrintReceipt agar bisa dimasukkan ke deps array dengan benar.
  ...
  // ✅ FIX: tambahkan handleFinishTransaction ke deps array
  ...
  const processCheckoutOrUpdate = async (e) => {
    ...
    try {
      ...
      if (!editingTransactionId) {
        ...
        cart.forEach(item => {
          // ✅ FIX: Gunakan increment() — atomic di Firestore sehingga
          // tidak ada race condition meski 2 kasir checkout bersamaan.
          // item.stock TIDAK dipakai karena stale (snapshot lama).
        ...
        allIds.forEach(id => {
          ...
          // ✅ FIX: increment(-delta) atomic — tidak pakai master.stock absolut
          // untuk menghindari race condition saat 2 edit terjadi bersamaan.
  ...
  const handleProcessUpdateStock = async (e) => {
    ...
    const batch  = writeBatch(db);
    // ✅ FIX: increment() atomic — tidak menulis nilai absolut yang stale.
    // Jika admin lain juga update stok bersamaan, tidak akan overwrite.
  ...
  // ✅ FIX: resumeHoldBill menggunakan setCart langsung (bukan
  // window.dispatchEvent yang tidak pernah didengar, dan bukan
  // useCart.__proto__ yang invalid). Juga pakai static firebase
  // imports yang sudah ada, bukan dynamic import + require().
  ...
  const dashboardProps = {
    ...
    // ✅ FIX: resumeHoldBill sekarang menggunakan setCart langsung
    // dan static firebase imports (deleteDoc, doc sudah di-import di atas)
    resumeHoldBill,
