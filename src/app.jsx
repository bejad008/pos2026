...
import { 
  ...
  ClipboardList, Download, Calendar, History
} from 'lucide-react';

// === IMPORT DARI FILE LOKAL (YANG SUDAH KITA PECAH) ===
...
export default function App() {
  ...
  // DIALOG UTILITY PENGGANTI PROMPT
  ...
  const handleReprintReceipt = (trx) => {
    ...
    setTimeout(() => { window.print(); }, 800);
    const rawReceiptText = generateRawTextReceipt(trx);
        ...
        allIds.forEach(id => {
            ...
            if (deltaQty !== 0) {
                ...
                if (deltaQty < 0) {
                    ...
                    logType = 'IN';
                    logReason = `Retur Item (Edit INV: ${oldTrx.id})`;
                    if (updatedStock > 0) {
                        updatedCost = ((productMaster.stock * productMaster.cost) + (qtyReturned * oldItem.cost)) / updatedStock;
                    }
                } else {
                    logType = 'OUT';
                    logReason = `Penambahan Item (Edit INV: ${oldTrx.id})`;
                }
  ...
  const cancelEditMode = () => {
    ...
    clearCart(); setDiscount(0); setIsCartOpen(false);
    setActiveTab('history');
  };
  ...
  return (
    <div className={`flex h-[100dvh] w-full text-amber-50 font-sans overflow-hidden relative selection:bg-red-800 ${printType === 'receipt' || printType === 'reprint' ? 'print-mode-receipt' : printType === 'report' ? 'print-mode-report' : ''}`}>
      ...
      {/* AREA CETAK TERSEMBUNYI LAPORAN A4 */}
      <div id="printable-report" className="hidden z-[9999]"></div>

      {/* APLIKASI UTAMA (Sembunyikan sepenuhnya saat print agar Android tidak blank) */}
      <div id="app-root-container" className={`flex h-[100dvh] w-full text-amber-50 overflow-hidden relative ${printType ? 'hidden' : 'block'}`}>
        ...
        <main className="flex-1 flex flex-col h-full z-10 relative min-w-0">
            ...
            <div className="flex-1 flex h-full min-h-0 relative">
              ...
              {editingTransactionId && (
                <div className="absolute top-0 left-0 right-0 bg-red-600 text-white text-center py-2 font-bold z-20 flex justify-center items-center gap-4 shadow-lg animate-pulse">
                  ...
                  <button onClick={cancelEditMode} className="bg-black/30 hover:bg-black/50 px-3 py-1 rounded text-xs">Batalkan Edit</button>
                </div>
              )}
              ...
              <div className={`flex-1 flex flex-col min-w-0 ${editingTransactionId ? 'pt-10' : ''}`}>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 pt-0 bg-transparent"><div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-20 md:pb-4 mt-2">{filteredProductsSafe.map(product => {
                  const isOutOfStock = (product.stock || 0) <= 0; 
                  return (
                    <div key={product.id} onClick={() => !isOutOfStock && addToCart(product)} className={`bg-[#261c19]/80 backdrop-blur-md border border-amber-700/30 rounded-2xl overflow-hidden flex flex-col h-full transition-all duration-300 relative p-4 shadow-lg ${isOutOfStock ? 'opacity-60 cursor-not-allowed grayscale' : 'cursor-pointer hover:border-red-500/50 hover:bg-[#30231f]/90 group'}`}>
                      <div className="flex justify-between items-start mb-2"><span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider">{product.category || 'Lain-lain'}</span><span className={`text-[10px] font-mono px-2 py-0.5 rounded ${isOutOfStock ? 'bg-red-900/80 text-red-200' : 'bg-amber-900/60 text-amber-200'}`}>Stok: {product.stock || 0}</span></div>
                      <h3 className={`font-semibold text-base leading-tight my-3 flex-1 text-amber-50 ${!isOutOfStock && 'group-hover:text-red-300'}`}>{product.name}</h3>
                      <div className="flex items-end justify-between mt-auto border-t border-amber-900/30 pt-3"><span className="text-amber-300 font-bold text-lg">{formatRupiah(product.price)}</span>{!isOutOfStock && (<button className="w-8 h-8 rounded-full bg-red-900/60 text-red-200 flex items-center justify-center group-hover:bg-red-600 group-hover:text-white transition-colors"><Plus size={16} /></button>)}</div>{isOutOfStock && <div className="absolute inset-0 flex items-center justify-center bg-black/70 font-bold text-red-500 uppercase tracking-widest text-lg border-2 border-red-500 rounded-2xl backdrop-blur-sm">Habis</div>}
                    </div>
                  )
                })}</div></div>
              </div>
              ...
              {/* KERANJANG PANEL */}
              <div className={`fixed lg:static inset-y-0 right-0 z-50 w-full max-w-[380px] bg-[#221815]/95 backdrop-blur-xl border-l border-red-900/30 flex flex-col shadow-2xl transition-transform duration-300 ${isCartOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'} shrink-0 ${editingTransactionId ? 'pt-10' : ''}`}>
                <div className="p-4 border-b border-red-900/20 flex justify-between items-center bg-[#1d1411]/80 shrink-0"><div className="flex items-center gap-2"><ShoppingCart className="text-amber-500" size={20} /><h2 className="font-bold text-amber-50 tracking-wide">Struk Pembelian</h2></div><div className="flex gap-1">{!editingTransactionId && <button onClick={handleHoldBill} disabled={cart.length===0} className="p-2 text-amber-400 hover:text-emerald-400 disabled:opacity-30" title="Simpan Meja"><PauseCircle size={18} /></button>}<button onClick={clearCart} disabled={cart.length===0} className="p-2 text-amber-500 hover:text-red-400 disabled:opacity-30"><Trash2 size={18} /></button><button onClick={() => setIsCartOpen(false)} className="lg:hidden p-2 text-amber-500 hover:text-white"><X size={20} /></button></div></div>
                <div className="flex-1 overflow-y-auto p-3 space-y-2">
                  {cart.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-amber-100/30"><ShoppingCart size={40} /><p className="mt-2 text-sm">Belum ada item</p></div>) : (cart.map(item => (<div key={item.id} className="flex gap-3 bg-[#2a1f1b]/80 p-3 rounded-xl border border-amber-900/30 items-center"><div className="flex-1 min-w-0"><h4 className="font-semibold text-amber-50 text-sm truncate">{item.name}</h4><div className="text-amber-400 text-xs mt-1">{formatRupiah(item.price)}</div></div><div className="flex items-center bg-[#1d1411]/80 rounded-lg border border-amber-700/30 shrink-0 h-8"><button onClick={() => updateQty(item.id, -1)} className="w-8 h-full flex items-center justify-center text-amber-400 hover:text-red-400"><Minus size={14} /></button><span className="w-6 text-center text-sm font-bold text-amber-50">{item.qty || 0}</span><button onClick={() => updateQty(item.id, 1)} className="w-8 h-full flex items-center justify-center text-amber-400 hover:text-emerald-400"><Plus size={14} /></button></div></div>)))}
                </div>
                <div className="bg-[#1d1411]/90 border-t border-red-900/30 p-4 shrink-0 space-y-3">
                  <div className="flex gap-2"><input type="number" placeholder="Potongan (Rp)" value={discount || ''} onChange={e => setDiscount(Number(e.target.value))} className="flex-1 bg-[#1a1412] border border-amber-700/50 rounded-lg p-2 text-amber-50 text-xs focus:outline-none"/><button onClick={() => setTaxEnabled(!taxEnabled)} className={`px-3 py-2 rounded-lg text-xs font-bold border transition-colors ${taxEnabled ? 'bg-red-800 text-white border-red-500' : 'bg-[#1a1412] text-amber-500 border-amber-700/50'}`}>+ PPN 10%</button></div>
                  <div className="space-y-1"><div className="flex justify-between text-xs text-amber-500/80"><span>Subtotal Dasar</span><span>{formatRupiah(subtotal)}</span></div>{discount > 0 && <div className="flex justify-between text-xs text-emerald-400"><span>Diskon Manual</span><span>-{formatRupiah(discount)}</span></div>}{taxEnabled && <div className="flex justify-between text-xs text-red-400"><span>Pajak Transaksi 10%</span><span>+{formatRupiah(taxAmount)}</span></div>}<div className="pt-2 border-t border-amber-900/30 flex justify-between items-center"><span className="text-sm font-bold">Total Pembayaran</span><span className="text-xl font-black text-amber-300">{formatRupiah(grandTotal)}</span></div></div>
                  <button onClick={() => setCheckoutModalOpen(true)} disabled={cart.length === 0} className={`w-full flex items-center justify-center gap-2 py-3 text-white rounded-xl font-bold transition-colors shadow-lg border disabled:opacity-50 disabled:cursor-not-allowed ${editingTransactionId ? 'bg-amber-700 hover:bg-amber-600 border-amber-500' : 'bg-red-800/90 hover:bg-red-700 border-red-600/50'}`}>{editingTransactionId ? <><Edit2 size={20} /> Update Transaksi</> : <><Banknote size={20} /> Checkout Transaksi</>}</button>
                </div>
              </div>
            </div>
          )}

          {/* TAB BARU: HISTORY TRANSAKSI (RIWAYAT & RETUR) */}
          {activeTab === 'history' && ['admin', 'kasir'].includes(currentUser?.role) && (() => {
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-black/40 backdrop-blur-sm">
                <div className="max-w-6xl mx-auto space-y-6">
                  <div className="flex flex-col xl:flex-row justify-between gap-4 mb-6">
                    <div className="flex items-center gap-3"><History className="text-red-500" size={28} /><h2 className="text-2xl font-bold text-amber-50">Riwayat & Retur Transaksi</h2></div>
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="bg-[#1a1412] p-1 border border-amber-900/50 rounded-xl flex items-center">
                        <button onClick={() => setHistoryFilter('today')} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition ${historyFilter === 'today' ? 'bg-red-900/80 text-white shadow-lg' : 'text-amber-500/70 hover:text-amber-300'}`}>Hari Ini</button>
                        <button onClick={() => setHistoryFilter('yesterday')} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition ${historyFilter === 'yesterday' ? 'bg-red-900/80 text-white shadow-lg' : 'text-amber-500/70 hover:text-amber-300'}`}>Kemarin</button>
                        <button onClick={() => setHistoryFilter('custom')} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition ${historyFilter === 'custom' ? 'bg-red-900/80 text-white shadow-lg' : 'text-amber-500/70 hover:text-amber-300'}`}>Pilih Tgl</button>
                      </div>
                      {historyFilter === 'custom' && (
                        <input type="date" value={historyCustomDate} onChange={e => setHistoryCustomDate(e.target.value)} className="bg-[#1a1412] border border-amber-700/50 text-amber-50 rounded-xl px-3 py-1.5 focus:outline-none"/>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-[#221815]/90 backdrop-blur-xl border border-amber-900/30 rounded-2xl overflow-hidden shadow-2xl">
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-sm">
                        <thead><tr className="bg-[#1d1411]/80 border-b border-red-900/30 text-amber-500"><th className="p-4">Waktu</th><th className="p-4">Invoice ID</th><th className="p-4">Tipe Bayar</th><th className="p-4 text-right">Nilai Transaksi</th><th className="p-4 text-center">Tindakan Kasir</th></tr></thead>
                        <tbody className="divide-y divide-amber-900/20">
                          {filteredHistory.map(trx => (
                            <tr key={trx.id} className="hover:bg-[#2a1f1b]/60 transition-colors">
                              <td className="p-4 text-xs text-amber-100">{new Date(trx.timestamp).toLocaleString('id-ID')}</td>
                              <td className="p-4 font-mono font-bold text-amber-50">{trx.id} {trx.lastEditedBy && <span className="block text-[10px] text-amber-500/50">Diubah: {trx.lastEditedBy}</span>}</td>
                              <td className="p-4"><span className="bg-[#1a1412] border border-amber-700/50 text-amber-400 px-2 py-1 rounded text-xs uppercase">{trx.paymentMethod}</span></td>
                          ...
                          {filteredHistory.length === 0 && (<tr><td colSpan="5" className="p-8 text-center text-amber-100/50">Tidak ada transaksi ditemukan pada tanggal ini.</td></tr>)}
                        </tbody>
                      </table>
                    </div>
                  </div>
          ...
          {/* TAB LAINNYA DIHIDE UNTUK SINGKAT */}
              ...
              <div className="max-w-5xl mx-auto space-y-6">
                ...
                {['admin', 'kasir'].includes(currentUser?.role) && (
                  <div className="bg-[#221815]/90 backdrop-blur-xl border border-amber-900/30 rounded-2xl p-6 shadow-2xl mb-6">
                    ...
                    <form onSubmit={handleAddCustomer} className="flex flex-col sm:flex-row gap-3"><input type="text" placeholder="Nama Entitas Pelanggan" value={newCustomerName} onChange={e => setNewCustomerName(e.target.value)} className="flex-1 bg-[#1a1412]/80 border border-amber-700/50 rounded-xl p-3 text-amber-50 focus:border-red-500 focus:outline-none"/><input type="text" placeholder="Nomor Telepon" value={newCustomerPhone} onChange={e => setNewCustomerPhone(e.target.value)} className="flex-1 bg-[#1a1412]/80 border border-amber-700/50 rounded-xl p-3 text-amber-50 focus:border-red-500 focus:outline-none"/><button type="submit" className="px-6 py-3 bg-red-800/90 border border-red-600/50 text-white rounded-xl font-bold whitespace-nowrap">Simpan Data</button></form>
                  </div>
                )}
                <div className="bg-[#221815]/90 backdrop-blur-xl border border-amber-900/30 rounded-2xl overflow-hidden shadow-2xl">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-amber-900/20">
                      {customers.map(c => (<tr key={c.id} className="hover:bg-[#2a1f1b]/60 transition-colors"><td className="p-4 font-bold text-amber-50">{c.name}</td><td className="p-4 text-sm text-amber-400">{c.phone || '-'}</td><td className="p-4 text-right font-bold text-red-400 text-lg">{formatRupiah(c.debt)}</td>{['admin', 'kasir'].includes(currentUser?.role) && (<td className="p-4 text-center"><button onClick={() => {setDebtCustomer(c); setPayDebtModalOpen(true);}} disabled={(c.debt || 0) <= 0} className="px-4 py-2 bg-emerald-800/80 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold disabled:opacity-30 border border-emerald-500/50">Lunasi Piutang</button></td>)}</tr>))}
                    </tbody>
                  </table>
                </div>
              </div>
              ...
              <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="bg-[#221815]/90 backdrop-blur-xl border border-amber-900/30 rounded-2xl overflow-hidden shadow-2xl">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-amber-900/20">{expenses.map(e => (<tr key={e.id} className="hover:bg-[#2a1f1b]/60 transition-colors"><td className="p-4 text-xs text-amber-100">{new Date(e.timestamp).toLocaleString('id-ID')}</td><td className="p-4 font-bold text-amber-50">{e.description}</td><td className="p-4 text-sm text-amber-400">{e.category}</td><td className="p-4 text-right font-bold text-red-400">{formatRupiah(e.amount)}</td><td className="p-4 text-xs text-amber-500/80">{e.kasirName}</td></tr>))}</tbody>
                  </table>
                </div>
              </div>
            </div>
              ...
              <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                  ...
                  {currentUser?.role === 'admin' && (
                    <div className="flex items-center gap-2"><button onClick={() => setIsCategoryModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-[#2a1f1b]/80 border border-amber-700/50 hover:bg-[#30231f] text-amber-100 rounded-lg font-semibold"><Tags size={18} /> Master Kategori</button><button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-red-800/90 border border-red-500/50 hover:bg-red-700 text-amber-50 rounded-lg font-semibold"><Plus size={18} /> Tambah SKU Baru</button></div>
                  )}
                </div>
                <div className="bg-[#221815]/90 backdrop-blur-xl border border-amber-900/30 rounded-2xl overflow-hidden shadow-2xl">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-amber-900/20">
                        {products.map(p => (<tr key={p.id} className="hover:bg-[#2a1f1b]/60 transition-colors"><td className="p-4 font-bold text-amber-50">{p.name}</td><td className="p-4 text-sm text-amber-400">{p.category}</td><td className="p-4 text-sm"><div className="text-amber-500/80">HPP Avg: <span className="text-amber-200">{formatRupiah(p.cost)}</span></div><div className="text-amber-400">Harga Jual: <span className="font-bold text-amber-300">{formatRupiah(p.price)}</span></div></td><td className="p-4 text-center"><span className={`text-lg font-black ${p.stock <= 5 ? 'text-red-500 animate-pulse' : 'text-emerald-400'}`}>{p.stock || 0}</span></td>{currentUser?.role === 'admin' && (<td className="p-4 text-center"><div className="flex items-center justify-center gap-2"><button onClick={() => openUpdateStockModal(p)} className="flex items-center gap-1 px-3 py-1.5 bg-emerald-800/80 border border-emerald-500/50 hover:bg-emerald-700 text-emerald-50 rounded-lg text-xs font-bold"><RefreshCw size={14} /> Mutasi</button><button onClick={() => handleDeleteItem(p.id)} className="p-1.5 text-amber-500 hover:text-red-400 hover:bg-red-900/20 rounded-lg"><Trash2 size={18} /></button></div></td>)}</tr>))}
                      </tbody>
                    </table>
                  </div>
                </div>
          ...
          {activeTab === 'stocklogs' && ['admin', 'owner'].includes(currentUser?.role) && (
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-black/40 backdrop-blur-sm">
              <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center justify-between gap-3 mb-6"><div className="flex items-center gap-3"><ClipboardList className="text-red-500" size={28} /><h2 className="text-2xl font-bold text-amber-50">Buku Mutasi Stok (Audit Trail)</h2></div><button onClick={() => downloadCSV(stockLogs.map(l => ({"Waktu": new Date(l.timestamp).toLocaleString('id-ID'), "Produk": l.productName, "Tipe": l.type, "Alasan": l.reason, "Perubahan Qty": l.qtyChange, "Stok Akhir": l.finalStock, "HPP": l.hpp, "User": l.user})), "Buku_Mutasi_Stok")} className="flex items-center gap-2 bg-[#2a1f1b] border border-amber-700/50 hover:bg-[#30231f] text-amber-300 px-4 py-2 rounded-xl text-sm font-bold transition"><Download size={16}/> Ekspor Excel</button></div>
                <div className="bg-[#221815]/90 backdrop-blur-xl border border-amber-900/30 rounded-2xl overflow-hidden shadow-2xl">
                  <table className="w-full text-left border-collapse text-sm">
          ...
          {activeTab === 'reports' && ['admin', 'owner'].includes(currentUser?.role) && (() => {
            const now = new Date(); const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime(); const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1).getTime(); const startOfYear = new Date(now.getFullYear(), 0, 1).getTime();
            ...
            const grossProfit = totalSales - totalCostGoods; const realNetProfit = grossProfit - totalExpense; const totalPiutang = customers.reduce((s,c) => s + (c.debt || 0), 0);
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-black/40 backdrop-blur-sm">
                <div className="max-w-6xl mx-auto space-y-6">
                  <div className="flex flex-col xl:flex-row justify-between gap-4 mb-6">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="bg-[#1a1412] p-1 border border-amber-900/50 rounded-xl flex items-center">{[ {id: 'hari_ini', label: 'Hari Ini'}, {id: 'bulan_ini', label: 'Bulan Ini'}, {id: 'tahun_ini', label: 'Tahun Ini'}, {id: 'semua', label: 'Semua'} ].map(p => (<button key={p.id} onClick={() => setReportPeriod(p.id)} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition ${reportPeriod === p.id ? 'bg-red-900/80 text-white shadow-lg' : 'text-amber-500/70 hover:text-amber-300'}`}>{p.label}</button>))}</div>
      ...
      {confirmDialog.isOpen && (
        <div className="fixed inset-0 z-[600] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-red-500/50 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl p-6 text-center"><AlertTriangle className="text-red-500 mx-auto mb-4" size={48} /><h3 className="text-lg font-bold text-amber-50 mb-2">{confirmDialog.title}</h3><p className="text-sm text-amber-400 mb-6">{confirmDialog.message}</p><div className="flex gap-2"><button onClick={() => setConfirmDialog({isOpen: false})} className="flex-1 p-3 rounded-xl border border-amber-700/50 text-amber-400 hover:bg-[#1d1411] font-semibold">Batal</button><button onClick={() => { confirmDialog.onConfirm(); setConfirmDialog({isOpen: false}); }} className="flex-1 p-3 rounded-xl bg-red-800/90 text-white font-bold hover:bg-red-700">Ya, Lanjutkan</button></div></div></div>
      )}
      {promptDialog.isOpen && (
        <div className="fixed inset-0 z-[600] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-amber-500/50 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl"><div className="p-4 bg-[#1d1411] border-b border-amber-900/30"><h3 className="text-lg font-bold text-amber-50">{promptDialog.title}</h3></div><div className="p-5"><p className="text-sm text-amber-400 mb-3">{promptDialog.message}</p><input autoFocus type="text" value={promptDialog.value} onChange={(e) => setPromptDialog({...promptDialog, value: e.target.value})} placeholder={promptDialog.placeholder} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-xl p-3 text-amber-50 focus:border-red-500 focus:outline-none mb-4"/><div className="flex gap-2"><button onClick={() => setPromptDialog({isOpen: false})} className="flex-1 p-3 rounded-xl border border-amber-700/50 text-amber-400 hover:bg-[#1d1411] font-semibold">Batal</button><button onClick={() => { promptDialog.onConfirm(promptDialog.value); setPromptDialog({isOpen: false}); }} className="flex-1 p-3 rounded-xl bg-red-800/90 text-white font-bold hover:bg-red-700">Simpan</button></div></div></div></div>
      )}

      {/* CHECKOUT MODAL */}
      {checkoutModalOpen && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-red-600/50 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"><div className="p-4 bg-[#1d1411] border-b border-red-900/30 flex justify-between items-center"><h3 className="text-lg font-bold text-amber-50">Sistem Pembayaran</h3><button onClick={() => setCheckoutModalOpen(false)} className="text-amber-500 hover:text-white"><X size={20}/></button></div><form onSubmit={processCheckoutOrUpdate} className="p-5 space-y-4"><div className="text-center bg-[#1a1412] border border-amber-700/30 p-4 rounded-xl"><p className="text-sm text-amber-500/80 uppercase">Tagihan Akhir</p><p className="text-3xl font-black text-amber-300 font-mono">{formatRupiah(grandTotal)}</p></div><div><label className="block text-xs font-semibold text-amber-400 mb-1">Instrumen Pembayaran</label><div className="flex gap-2">{['Tunai', 'QRIS', 'Tempo'].map(m => <button key={m} type="button" onClick={() => setPaymentMethod(m)} className={`flex-1 py-2 rounded-lg font-bold text-sm border ${paymentMethod === m ? 'bg-red-800 text-white border-red-500' : 'bg-[#1a1412] text-amber-500 border-amber-700/50 hover:bg-[#30231f]'}`}>{m}</button>)}</div></div>{paymentMethod === 'Tempo' && (<div className="bg-red-900/20 border border-red-700/30 p-3 rounded-xl"><label className="block text-xs font-semibold text-red-400 mb-1">Bebankan Ke Pelanggan:</label><select required value={selectedCustomerId} onChange={e => setSelectedCustomerId(e.target.value)} className="w-full bg-[#1a1412] border border-red-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"><option value="">-- Pilih Relasi --</option>{customers.map(c => <option key={c.id} value={c.id}>{c.name} (Hutang: {formatRupiah(c.debt)})</option>)}</select></div>)}{paymentMethod === 'Tunai' && !editingTransactionId && (<div><label className="block text-xs font-semibold text-amber-400 mb-1">Kas Diterima (Rp)</label><input required type="number" min={grandTotal} value={amountTendered} onChange={e => setAmountTendered(e.target.value)} className="w-full bg-[#1a1412] border border-emerald-700/50 rounded-lg p-3 text-emerald-400 text-lg font-bold text-center focus:outline-none"/>{Number(amountTendered) >= grandTotal && <p className="text-center text-sm font-bold text-emerald-400 mt-2">Uang Kembali: {formatRupiah(Number(amountTendered) - grandTotal)}</p>}</div>)}<button type="submit" className={`w-full p-3 rounded-xl text-white font-bold text-lg mt-4 shadow-lg ${editingTransactionId ? 'bg-amber-700 hover:bg-amber-600' : 'bg-emerald-800/90 hover:bg-emerald-700'}`}>{editingTransactionId ? 'Selesaikan Update & Retur' : 'Selesaikan Pembayaran'}</button></form></div></div>
      )}

      {/* UPDATE STOK MODAL */}
      {isStockModalOpen && stockUpdateItem && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-emerald-600/50 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl"><div className="p-4 bg-[#1d1411] border-b border-emerald-900/30 flex justify-between items-center"><h3 className="text-lg font-bold text-amber-50 flex items-center gap-2"><RefreshCw size={20}/> Mutasi Inventaris</h3><button onClick={() => setIsStockModalOpen(false)} className="text-amber-500 hover:text-white"><X size={20}/></button></div><form onSubmit={handleProcessUpdateStock} className="p-5 space-y-4"><div className="text-center mb-2"><p className="text-sm text-amber-400">SKU: <strong className="text-amber-100 block text-lg">{stockUpdateItem.name}</strong></p><p className="text-xs text-emerald-400 font-mono bg-emerald-900/20 py-1 rounded inline-block px-3 mt-1">Stok Tercatat: {stockUpdateItem.stock || 0} | HPP Lama: {formatRupiah(stockUpdateItem.cost)}</p></div><div><label className="block text-xs font-semibold text-amber-500 mb-1 text-center">Tipe Mutasi</label><select value={stockUpdateReason} onChange={e => setStockUpdateReason(e.target.value)} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none mb-3"><option value="Restock Barang Masuk">Restock (Barang Masuk)</option><option value="Koreksi Barang Rusak/Expired">Koreksi Rusak/Expired (Pengurangan)</option><option value="Audit Stok Opname">Audit Stok Opname</option></select><label className="block text-xs font-semibold text-amber-500 mb-1 text-center">Volume / Jumlah Barang</label><input autoFocus required type="number" placeholder="Contoh: 10 atau -5" value={stockUpdateAmount} onChange={e => setStockUpdateAmount(e.target.value)} className="w-full bg-[#1a1412] border border-emerald-700/50 rounded-xl p-3 text-emerald-400 font-bold text-center focus:outline-none text-lg"/></div>{Number(stockUpdateAmount) > 0 && stockUpdateReason === 'Restock Barang Masuk' && (<div className="bg-emerald-900/10 border border-emerald-900/30 p-3 rounded-xl"><label className="block text-xs font-semibold text-emerald-400 mb-2 text-center">Input Harga Beli Baru (Satuan) Untuk HPP Average</label><input required type="number" min="0" value={newCostPrice} onChange={e => setNewCostPrice(e.target.value)} className="w-full bg-[#1a1412] border border-emerald-700/50 rounded-xl p-3 text-emerald-400 font-bold text-center focus:outline-none"/></div>)}<button type="submit" className="w-full p-3 rounded-xl bg-emerald-800/90 hover:bg-emerald-700 text-white font-bold mt-2">Simpan Pembukuan Stok</button></form></div></div>
      )}

      {/* TAMBAH MENU MODAL */}
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-red-600/50 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"><div className="p-4 bg-[#1d1411] border-b border-red-900/30 flex justify-between items-center"><h3 className="text-lg font-bold text-amber-50">Registrasi Master Item</h3><button onClick={() => setIsAddModalOpen(false)} className="text-amber-500 hover:text-white"><X size={20}/></button></div><form onSubmit={handleAddProduct} className="p-5 space-y-4"><div><label className="block text-xs text-amber-400 mb-1">Nama SKU/Item</label><input required type="text" value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"/></div><div><label className="block text-xs text-amber-400 mb-1">Kategori Induk</label><select required value={newItem.category} onChange={e => setNewItem({...newItem, category: e.target.value})} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"><option value="">-- Pilih --</option>{categories.map(c => <option key={c} value={c}>{c}</option>)}</select></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-xs text-amber-400 mb-1">HPP Awal (Rp)</label><input required type="number" min="0" value={newItem.cost} onChange={e => setNewItem({...newItem, cost: e.target.value})} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"/></div><div><label className="block text-xs text-amber-400 mb-1">Harga Jual (Rp)</label><input required type="number" min="0" value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"/></div></div><div><label className="block text-xs text-amber-400 mb-1">Inisiasi Volume Stok</label><input required type="number" min="0" value={newItem.stock} onChange={e => setNewItem({...newItem, stock: e.target.value})} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"/></div><button type="submit" className="w-full p-3 rounded-xl bg-red-800/90 text-white font-bold text-sm mt-2">Publish ke Server</button></form></div></div>
      )}

      {/* KELOLA KATEGORI */}
      {isCategoryModalOpen && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-red-600/50 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"><div className="p-4 bg-[#1d1411] border-b border-red-900/30 flex justify-between items-center"><h3 className="text-lg font-bold text-amber-50 flex items-center gap-2"><Tags size={20}/> Kelola Kategori</h3><button onClick={() => setIsCategoryModalOpen(false)} className="text-amber-500 hover:text-white"><X size={20}/></button></div><div className="p-5 space-y-4"><form onSubmit={handleAddCategory} className="flex gap-2"><input required autoFocus type="text" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} placeholder="Nama kategori baru..." className="flex-1 bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none focus:border-red-500"/><button type="submit" className="px-4 py-2 bg-red-800/90 text-white rounded-lg font-bold text-sm shadow-lg">Tambah</button></form><div className="bg-[#1a1412] border border-amber-900/30 rounded-xl overflow-hidden max-h-60 overflow-y-auto"><ul className="divide-y divide-amber-900/20">{categories.map(cat => (<li key={cat} className="p-3 flex justify-between items-center hover:bg-[#2a1f1b]/60 transition-colors"><span className="text-amber-100 font-medium text-sm">{cat}</span><button onClick={() => handleRemoveCategory(cat)} className="p-1.5 text-amber-500 hover:text-red-400 hover:bg-red-900/20 rounded-lg transition-colors"><Trash2 size={16} /></button></li>))}{categories.length === 0 && <li className="p-4 text-center text-sm text-amber-500/50">Belum ada kategori kustom.</li>}</ul></div></div></div></div>
      )}

      {/* SHIFT MODAL */}
      {shiftModalOpen && (
        <div className="fixed inset-0 z-[400] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4"><div className="bg-[#221815] border-2 border-amber-500/80 rounded-2xl w-full max-w-sm overflow-hidden shadow-[0_0_50px_rgba(245,158,11,0.15)]"><div className="p-4 bg-[#1d1411] border-b border-amber-900/50 text-center"><h3 className="text-xl font-black text-amber-400 uppercase tracking-widest">{shiftAction === 'open' ? 'BUKA LACI KASIR' : 'TUTUP LACI KASIR'}</h3></div><form onSubmit={handleProcessShift} className="p-6 space-y-5"><p className="text-sm text-amber-500/80 text-center">{shiftAction === 'open' ? `Halo ${currentUser?.name || 'Kasir'}! Masukkan uang modal laci Anda untuk mulai shift.` : 'Masukkan total uang fisik yang ada di dalam laci kasir saat ini.'}</p><div><label className="block text-xs font-semibold text-amber-500 mb-2 text-center">{shiftAction === 'open' ? 'MODAL AWAL SHIFT (Rp)' : 'TOTAL UANG FISIK (Rp)'}</label><input required autoFocus type="number" min="0" placeholder="Rp 0" value={shiftCashInput} onChange={e => setShiftCashInput(e.target.value)} className="w-full bg-[#1a1412] border-2 border-amber-700/50 rounded-xl p-4 text-amber-50 text-2xl font-bold text-center focus:border-amber-400 focus:outline-none"/></div><button type="submit" className="w-full p-4 rounded-xl bg-gradient-to-r from-amber-700 to-amber-900 hover:from-amber-600 hover:to-amber-800 border border-amber-500/50 text-white font-black transition-all text-lg shadow-xl uppercase tracking-widest">{shiftAction === 'open' ? 'Mulai Bekerja' : 'Laporkan & Tutup'}</button></form></div></div>
      )}

      {/* MODAL PELUNASAN HUTANG */}
      {payDebtModalOpen && debtCustomer && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-emerald-600/50 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl"><div className="p-4 bg-[#1d1411] border-b border-emerald-900/30 flex justify-between items-center"><h3 className="text-lg font-bold text-amber-50">Pelunasan Piutang</h3><button onClick={() => setPayDebtModalOpen(false)} className="text-amber-500 hover:text-white"><X size={20}/></button></div><form onSubmit={processPayDebt} className="p-5 space-y-4 text-center"><p className="text-sm text-amber-400">Pembayaran untuk: <strong className="text-amber-100 block text-lg">{debtCustomer.name}</strong></p><p className="text-xs text-red-400">Sisa Hutang: {formatRupiah(debtCustomer.debt)}</p><div><input autoFocus required type="number" min="1" max={debtCustomer.debt} placeholder="Nominal Uang (Rp)" value={payDebtAmount} onChange={e => setPayDebtAmount(e.target.value)} className="w-full bg-[#1a1412] border border-emerald-700/50 rounded-xl p-3 text-emerald-400 font-bold text-center focus:border-emerald-500 focus:outline-none text-lg"/></div><button type="submit" className="w-full p-3 rounded-xl bg-emerald-800/90 hover:bg-emerald-700 text-white font-bold transition-all border border-emerald-600/50">Konfirmasi Pembayaran Tunai</button></form></div></div>
      )}

      {/* MODAL CATAT PENGELUARAN */}
      {expenseModalOpen && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-red-600/50 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"><div className="p-4 bg-[#1d1411] border-b border-red-900/30 flex justify-between items-center"><h3 className="text-lg font-bold text-amber-50 flex items-center gap-2"><Wallet size={20} className="text-red-500" /> Catat Kas Keluar</h3><button onClick={() => setExpenseModalOpen(false)} className="text-amber-500 hover:text-white"><X size={20}/></button></div><form onSubmit={handleAddExpense} className="p-5 space-y-4"><div><label className="block text-xs font-semibold text-amber-400 mb-1">Keterangan / Tujuan Kas</label><input required type="text" placeholder="Misal: Beli Token Listrik" value={newExpense.description} onChange={e => setNewExpense({...newExpense, description: e.target.value})} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"/></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-semibold text-amber-400 mb-1">Kategori Biaya</label><select value={newExpense.category} onChange={e => setNewExpense({...newExpense, category: e.target.value})} className="w-full bg-[#1a1412] border border-amber-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"><option value="Operasional">Operasional (Listrik dll)</option><option value="Gaji">Gaji Karyawan</option><option value="Lain-lain">Lain-lain</option></select></div><div><label className="block text-xs font-semibold text-amber-400 mb-1">Nominal (Rp)</label><input required type="number" min="1" placeholder="0" value={newExpense.amount} onChange={e => setNewExpense({...newExpense, amount: e.target.value})} className="w-full bg-[#1a1412] border border-red-700/50 rounded-lg p-2.5 text-amber-50 text-sm focus:outline-none"/></div></div><button type="submit" className="w-full p-3 rounded-xl bg-red-800/90 hover:bg-red-700 text-white font-bold transition-all text-sm mt-2">Simpan Pengeluaran</button></form></div></div>
      )}
        ...
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#221815] border border-emerald-600/50 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl p-6 text-center">
            ...
            <div className="flex flex-col gap-3 mt-4">
              ...
              <button onClick={handleFinishTransaction} className="w-full p-4 rounded-xl border border-amber-700/50 text-amber-400 hover:bg-[#1d1411] font-semibold">Selesai Tanpa Cetak</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ALERT GLOBAL */}
      {alertMsg && (
        <div className="fixed inset-0 z-[700] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#221815] border border-amber-500/50 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl p-6 text-center"><h3 className="text-lg font-bold text-amber-50 mb-4 whitespace-pre-wrap">{alertMsg}</h3><button onClick={() => setAlertMsg("")} className="w-full p-3 rounded-xl bg-red-800/90 text-white font-bold hover:bg-red-700">Tutup Notifikasi</button></div></div>
      )}
