 ...
 * Styled untuk browser print dengan CSS @media print
 */
  ...
  return (
    <div id="printable-receipt" style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '4px' }}>
        <div style={{ fontSize: '16px', fontWeight: 'bold', letterSpacing: '2px' }}>{storeName}</div>
      ...
      <div style={{ fontSize: '11px', marginBottom: '4px' }}>
        ...
        {trx.shiftId && <div>Shift: {trx.shiftId.slice(-8)}</div>}
      </div>
      ...
      <table style={{ width: '100%', fontSize: '11px', borderCollapse: 'collapse' }}>
        <tbody>
          {(trx.items ?? []).map((item, i) => (
            <tr key={i}>
              <td colSpan={2} style={{ paddingBottom: '2px', fontWeight: 'bold' }}>
                {item.name}
              </td>
            </tr>
          )).concat(
            (trx.items ?? []).map((item, i) => (
              <tr key={`sub-${i}`}>
                <td style={{ paddingBottom: '4px', paddingLeft: '8px', color: '#333' }}>
                  {item.qty} x {Number(item.price).toLocaleString('id-ID')}
                </td>
                <td style={{ textAlign: 'right', paddingBottom: '4px', fontWeight: 'bold' }}>
      ...
      <table style={{ width: '100%', fontSize: '11px', borderCollapse: 'collapse' }}>
        <tbody>
          <tr>
            <td>Subtotal</td>
            <td style={{ textAlign: 'right' }}>{Number(trx.subtotal).toLocaleString('id-ID')}</td>
          </tr>
          {(trx.discount > 0) && (
            <tr>
              <td>Diskon</td>
              <td style={{ textAlign: 'right' }}>-{Number(trx.discount).toLocaleString('id-ID')}</td>
            </tr>
          )}
          {(trx.tax > 0) && (
            <tr>
              <td>PPN 10%</td>
              <td style={{ textAlign: 'right' }}>+{Number(trx.tax).toLocaleString('id-ID')}</td>
            </tr>
          )}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <tbody>
          ...
          <tr>
            <td style={{ fontSize: '11px' }}>Metode Bayar</td>
            <td style={{ textAlign: 'right', fontSize: '11px' }}>{trx.paymentMethod}</td>
          </tr>
          {trx.paymentMethod === 'Tempo' && (
            <tr>
              <td colSpan={2} style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '11px', color: '#cc0000', paddingTop: '2px' }}>
 ...
 * Untuk cetak laporan laba rugi via browser
 */
export function ReportDocument({ data, period }) {
  if (!data) return null;

  const { totalSales, totalCostGoods, grossProfit, totalExpense, realNetProfit, filteredTrx, filteredExp, products } = data;
  ...
  return (
    <div id="printable-report" style={{
      ...
      {/* Header */}
      <div style={{ textAlign: 'center', borderBottom: '3px solid #000', paddingBottom: '12px', marginBottom: '16px' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
        <tbody>
          ...
          <tr style={{ backgroundColor: realNetProfit >= 0 ? '#e8ffe8' : '#ffe8e8' }}>
            ...
            <td style={{ padding: '12px', border: '2px solid #000', textAlign: 'right', fontSize: '18px', fontWeight: 'bold', color: realNetProfit >= 0 ? '#008800' : '#cc0000' }}>
              {fmt(realNetProfit)}
            </td>
          </tr>
        </tbody>
      ...
      {topItems.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 'bold', borderBottom: '2px solid #000', paddingBottom: '4px', marginBottom: '8px' }}>
            Produk Terlaris
          </h3>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
            ...
            <tbody>
              {(filteredExp ?? []).map((e, i) => (
                <tr key={i} style={{ backgroundColor: i % 2 === 0 ? '#fff5f5' : '#fff' }}>
                  ...
                  <td style={{ padding: '5px 8px', border: '1px solid #ddd', textAlign: 'right', color: '#cc0000', fontWeight: 'bold' }}>
                    {fmt(e.amount)}
                  </td>
                </tr>
              ))}
      ...
      {/* Footer */}
      <div style={{ marginTop: '30px', borderTop: '1px solid #000', paddingTop: '8px', textAlign: 'center', fontSize: '11px', color: '#555' }}>
